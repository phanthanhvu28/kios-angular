...
import UserDto, { CreateUserRequest, DataFilterUser, DeleteUserRequest, ResetPassRequest, UpdateMenuRequest, UpdateRoleRequest, UpdateUserRequest } from '../models/user.model';
...
export class UserService extends BaseDataListService<UserDto> {
  ...
  override getPayload(): DataListRequestPayload {
    ...
    resetPass(payload:ResetPassRequest):void{
      this.setLoading(true);
      this._api.resetPass(payload)
      .pipe(
        takeUntil(this.destroy$),
        finalize(() => this.setLoading(false)),
        catchError((err) => {
