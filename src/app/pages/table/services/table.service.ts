...
export class TableService extends BaseDataListService<TableBaseDto>{
  ...
  private subjectCreateTable = new BehaviorSubject<any>(null);
  createTable$: Observable<ResultModel<TableBaseDto>> = this.subjectCreateTable;
  ...
  create(payload:TableRequest):void{
    ...
    .subscribe((res) => {
      ...
      this.subjectCreateTable.next(res);
      if (res?.isError) {
